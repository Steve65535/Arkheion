/**
 * FSCA CLI File Logger
 *
 * Intercepts console.log / console.warn / console.error and writes
 * timestamped entries to logs/<date>.log in the project root.
 *
 * Safe against:
 *   - Double-attach (idempotent — second call is a no-op)
 *   - Circular object references (uses safe serializer)
 *   - Stack overflow from recursive console patching
 */
...
function attachFileLogger(rootDir) {
    // Idempotent — only attach once per process
    if (_attached) return;
    _attached = true;
    ...
    // Redact sensitive argv before writing session header
    const safeArgv = process.argv.slice(2).map(arg => {
        // Redact --accountPrivateKey=... and --privateKey=...
        return arg.replace(/(--(?:accountPrivateKey|privateKey|private[-_]?key)=)\S+/i, '$1[REDACTED]');
    }).join(' ');

    _stream.write(`\n${'='.repeat(60)}\n`);
    _stream.write(`[${timestamp()}] [SESSION START] fsca ${safeArgv}\n`);
